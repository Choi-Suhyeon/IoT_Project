...
**타입 매개변수** : 위 코드에서의 `<W: Write>`. 임의의 타입을 나타냄. 보통 대문자 하나로 나타내는 것이 관례.

**바운드** : `<W: Write>`에서의 `Write`. 임의의 타입이 할 수 있는 일을 나타냄.
...
**where 절** : 바운드 부분이 너무 길어지면, 그 안에 있는 내용이 쉽게 눈에 안 들어옴. 이에 Rust에서는 where 절이라는 대체 문법을 제공함.
```rust
// where 절을 사용하지 않은 함수
...
**상수 매개변수** : 타입 외에 상수도 넘겨줄 수 있음. 이렇게 넘겨진 상수는 시그니처 또는 본문 내부 타입 안에 사용 가능.
```rust
...
**앵글 브레킷 내부 순서** : 앵글 브레킷 내부에는 타입 매개변수, 상수 매개변수, 수명 매개변수 이렇게 세 가지가 올 수 있음. 두 종류 이상이 올 때 순서는 반드시 수명 매개변수가 앞에 와야 함. 타입 매개변수와 상수 매개변수는 섞여도 상관 없음.
...
Kotlin 등에서 interface의 기본 메서드와 개념적으로 비슷하다고 볼 수 있음. 하지만, 내부적으로 봤을 때는 이들은 어떤 메서드를 호출할지를 컴파일 시간에 결정함. Rust는 이를 runtime 때 결정.
...
내부에서 `Self`를 사용하는 trait는 trait 객체로 만들 수가 없음. trait 객체의 핵심은 runtime에 해당 타입을 알 수 있으므로, compile time에는 `Self`의 타입이 같은지 다른지 알 수가 없음. 그래서 아래 두 코드는 서로 의미가 다름.
```rust
...
즉, subtrait는 Self의 바운드에 대한 문법적 설탕에 불과. 아래 두 코드는 완벽히 동일한 의미를 갖는 코드. 그렇기 때문에 여러 supertrait를 상속 받으려면 `+` 기호로 연결해 쓰면 됨.
```rust
