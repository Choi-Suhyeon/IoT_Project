...
- operator code : 명령어가 수행할 연산 (뭘 해라).
- operand : 주소 필드라고도 함. 연산에 사용할 데이터 (뭐로, 어디로부터, 어디에, ...).
    - 주소 필드라고도 하는 이유 : 레지스터 이름이나 메모리 주소 등 연산에 사용할 데이터가 있는 위치가 종종 담겨서

**x address instruction** : operand의 개수에 따라 하나도 없는 것은 0 address instruction, operand가 1개면 1 address instruction ...
- 주의 : 예를 들어, `incq %rax`와 같은 명령어를 봤을 때 `incq` 하나 `%rax` 하나, 따라서 2 address instruction이 아님! 그럼 공백으로 구분해서 '개수 - 1'이냐? 그것도 아님. x64dbg열면 assembly와 함께 opcode가 나오는데 그거보면 안 그런 경우도 있다는 것을 알 수 있음.
...
특수한 쓰임 : 레지스터가 아래의 대응되는 상황에서 쓰인다는 것이지 저 상황에서만 쓰이는 것은 아님.
- `%rax` : 함수의 반환 값.
...
C언어의 배열을 생각해보면 `arr[i]`(`arr`은 `int`를 담는 배열)로 접근함. 이걸 포인터 표현으로 변경하면 `*(arr + i)`임. 아래는 대응.
- $()$ : 코드에서의 `*()`과 같음.
- $r_b$ : `arr`은 배열의 이름으로 배열의 시작주소.
- $r_i$ : `i`는 인덱스. 
- $s$ : 중요한 것은 C언어에서 `ptr + 1`은 단순히 주소에 1을 더한 것이 아니라 포인터의 해석이 결정하는 크기를 더함 (`int` 포인터면 주소에 4byte를 더함). 즉, `*(arr + i)`는 `*((char *)arr + i * sizeof(int))`와 같음. 즉, 방금 코드에서의 `sizeof(int)`가 $s$임.
    - $s$에는 1($2^0$), 2($2^1$), 4($2^2$), 8($2^3$)밖에 올 수 없음. 위에서 다 설명해서 생각해보면 당연함.

물론 여기서는 `arr`의 자료형이 `int *`이라 연산이 끝나고 역참조를 하면 `int`크기만큼 가져와서 `int`로 해석하겠지만, 방금 봤던 어셈블리에서는 그런 것이 없음. `void *`임. 그래서 명령어에 operand의 크기를 나타내는 접미사가 붙음.
